		...
		function check( $php_files, $css_files, $other_files ) {
			...
			foreach ( $php_files as $file_path => $file_content ) {
				...
				if ( preg_match( $php_regex, $file_content, $matches ) ) {
					$this->error[] = sprintf( '<span class="tc-lead tc-required">' . __( 'REQUIRED', 'theme-check').'</span>: ' . __( 'PHP: You are not allowed to hide the admin bar.', 'theme-check' ), 
			...
			foreach ( $css_files as $file_path => $file_content ) {
				...
				if ( preg_match( $css_regex, $file_content, $matches ) ) {
					$this->error[] = sprintf( '<span class="tc-lead tc-required">' . __( 'REQUIRED', 'theme-check').'</span>: ' . __( 'CSS: You are not allowed to hide the admin bar.', 'theme-check' ), 
						'<strong>' . $filename . '</strong>');	
					$ret = false;			
				}
			}
