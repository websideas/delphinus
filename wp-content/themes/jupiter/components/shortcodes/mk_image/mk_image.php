...
$output.= '<img class="lightbox-' . $lightbox . '" alt="' . $image_alt . '" title="' . $image_title . '" width="' . $actual_image_width . '" height="' . $actual_image_height . '" src="' . $image_output_src . '" ' . $image_svg_enabled . '/>';

$output.= ($link) ? '</a>' : '';

if ($lightbox == 'true') {
    $output.= '<div class="mk-image-overlay"></div>';
    $output.= '<a href="' . $src_lightbox . '" alt="' . $image_alt . '" data-fancybox-group="image-shortcode-' . $group . '" title="' . $title . '" class="mk-lightbox ' . $group . ' mk-image-lightbox"><i class="mk-jupiter-icon-plus-circle"></i></a>';
}
