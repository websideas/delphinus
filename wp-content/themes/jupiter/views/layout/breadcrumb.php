...
if (function_exists('yoast_breadcrumb')) {
    ...
    if (function_exists('is_woocommerce') && is_woocommerce() && is_archive()) {
        ...
        if ($shop_page_id && $shop_page && get_option('page_on_front') !== $shop_page_id) {
            echo '<a href="' . get_permalink($shop_page) . '" rel="v:url" property="v:title">' . $shop_page->post_title . '</a> ';
        }
    }
    ...
    if (is_category() && !is_singular('portfolio')) {
        ...
        if (function_exists('is_woocommerce') && is_woocommerce() && get_post_type() == 'product') {
            ...
            if ($terms = wc_get_product_terms($post->ID, 'product_cat', array(
                ...
                foreach ($ancestors as $ancestor) {
                    ...
                    if (!is_wp_error($ancestor) && $ancestor) echo '<a href="' . get_term_link($ancestor->slug, 'product_cat') . '" rel="v:url" property="v:title">' . $ancestor->name . '</a>' . $delimiter;
                }
                
                echo '<a href="' . get_term_link($main_term->slug, 'product_cat') . '" rel="v:url" property="v:title">' . $main_term->name . '</a>' . $delimiter;
            }
        ...
        elseif (get_post_type() != 'post') {
            ...
            if (function_exists('is_bbpress') && is_bbpress()) {
                ...
                echo '<a href="' . get_post_type_archive_link(get_post_type()) . '" rel="v:url" property="v:title">' . $post_type->labels->singular_name . '</a>' . $delimiter;
                echo get_the_title();
            }
        } 
        else {
    ...
    if (function_exists('is_bbpress') && is_bbpress()) {
        ...
        if (!empty($post_type_object->has_archive) && !bbp_is_forum_archive()) {
            $item[] = '<a href="' . get_post_type_archive_link(bbp_get_forum_post_type()) . '" rel="v:url" property="v:title">' . bbp_get_forum_archive_title() . '</a>';
        }
        ...
        elseif (bbp_is_single_topic()) {
            ...
            if (bbp_is_topic_split() || bbp_is_topic_merge() || bbp_is_topic_edit()) $item[] = '<a href="' . bbp_get_topic_permalink($topic_id) . '" rel="v:url" property="v:title">' . bbp_get_topic_title($topic_id) . '</a>';
        ...
        elseif (bbp_is_single_reply()) {
            ...
            if (!bbp_is_reply_edit()) {
                ...
                $item[] = '<a href="' . bbp_get_reply_url($reply_id) . '" rel="v:url" property="v:title">' . bbp_get_reply_title($reply_id) . '</a>';
        ...
        elseif (bbp_is_single_user() || bbp_is_single_user_edit()) {
            ...
            if (bbp_is_single_user_edit()) {
                $item[] = '<a href="' . bbp_get_user_profile_url() . '" rel="v:url" property="v:title">' . bbp_get_displayed_user_field('display_name') . '</a>';
                $item[] = __('Edit', 'mk_framework');
            } 
            else {
