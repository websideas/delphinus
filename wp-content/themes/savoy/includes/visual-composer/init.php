<?php
	...
	if ( class_exists( 'WPBakeryVisualComposerAbstract' ) ) {
		 ...
		 */
		function nm_addFrontCss() {
			if ( is_shop() || is_product_taxonomy() ) {
				global $nm_globals;
				
				// Get custom styles from the post meta (returns empty strings if no results)
				$post_custom_css = get_post_meta( $nm_globals['shop_page_id'], '_wpb_post_custom_css', true );
				$shortcodes_custom_css = get_post_meta( $nm_globals['shop_page_id'], '_wpb_shortcodes_custom_css', true );
		...
		if ( nm_woocommerce_activated() ) {
			add_action( 'wp_head', 'nm_addFrontCss', 1000 );
		}
