...
if (!function_exists('kt_get_product_layout')) {
...
function kt_wc_widget_field_colors($key, $value, $setting, $instance)
{
    ...
    if (!empty($terms) && !is_wp_error($terms)) {
        ...
        $output = sprintf('<p>%s</p>', wp_kses(__('No product attribute saved with the <strong>"color"</strong> slug yet.', 'delphinus'), array('strong' => array())));
    }
    ...
    <ul class="gridlist-toggle">
        <li>
            <a class="grid<?php if ($gridlist == 'grid') { ?> active<?php } ?>" data-toggle="tooltip" href="#"
               title="<?php _e('Grid view', 'delphinus') ?>" data-layout="products-grid" data-remove="products-list">
                <i class="fa fa-th"></i>
            </a>
        </li>
        <li>
            <a class="list<?php if ($gridlist == 'list') { ?> active<?php } ?>" data-toggle="tooltip" href="#"
               title="<?php _e('List view', 'delphinus') ?>" data-layout="products-list" data-remove="products-grid">
...
function kt_category_menu()
{
    ...
    echo $shop_header_filters . '<div class="shop-header-left"><ul id="shop-header-categories" class="shop-header-list">' . $output . '</ul></div>' . $filters_html;
...
function kt_template_loop_product_thumbnail()
{
    ...
    if ($woocommerce_loop['type'] == 'transparent') {
        ...
        $thumbnail_product = kt_get_single_file('_kt_gallery', $image_size, $product->id);
        if ($thumbnail_product) {
            @list($width, $height) = getimagesize($thumbnail_product['url']);
            $thumbnail = '<img src="' . $thumbnail_product['url'] . '" alt="' . esc_attr(get_the_title()) . '" width="' . esc_attr($width) . '" class="wp-post-image" height="' . esc_attr($height) . '" />';
        }
